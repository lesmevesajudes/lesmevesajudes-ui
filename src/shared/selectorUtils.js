...
export const esFamiliaNombrosa = (persones: Map<AdultId, Adult>) =>{
    ...
    const dosFillsOMesAlgunAmbDiscapacitatSuperior33: boolean = persones.filter( (persona: Adult) => esFill(persona) ).count() >= 2 && persones.filter( (persona: Adult) => esFill(persona) && persona.grau_discapacitat > 33 ).count() >= 1;
